...
$stmt = $pdo->prepare("
    ...
    WHERE 
        af.status = 'approve' -- Filter for rows where rp_proposal_berf_files.status is 'approve'
    GROUP BY 
...
<div class="col-md-9 main-content">
    <h3 class="mb-3">Research Papers/Proposal/Berf</h3>
    ...
    <tbody>
        ...
        <?php foreach ($files as $file): ?>
            <tr>
                ...
                <td>
                    <button class="btn btn-info btn-sm preview-file" data-id="<?= $file['id'] ?>" data-bs-toggle="modal" data-bs-target="#previewFileModal">
                        <i class="fas fa-eye"></i>
                    </button>
                    <button class="btn btn-success btn-sm edit-file" data-id="<?= $file['id'] ?>" data-bs-toggle="modal" data-bs-target="#editFileModal">
                        <i class="fas fa-edit"></i>
                    </button>
                    <!-- Add button for file revision -->
                    <button class="btn btn-primary btn-sm add-revision" data-id="<?= $file['id'] ?>" data-bs-toggle="modal" data-bs-target="#addFileRevisionModal">
                        <i class="fas fa-plus"></i>
                    </button>
                    <?php if ($_SESSION['user_role'] !== 'faculty'): ?>
                        <button class="btn btn-danger btn-sm delete-file" data-id="<?= $file['id'] ?>">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="addFileForm" enctype="multipart/form-data" action="../functions/rp_proposal_berf_files/add_file.php" method="POST">
                    <div class="mb-4">
...
<!-- <script>
    $(document).ready(function () {
        ...
        $('.preview-file').on('click', function () {
            ...
            $.get('../functions/rp_proposal_berf_files/fetch_revisions.php', { file_id: fileId }, function (response) {
                        ...
                        revisions += `
                            <tr>
                                ...
                                <td>
                                    ...
                                    <button class="btn btn-primary btn-sm download-file" data-url="${revision.download_path}">
                                    <i class="fas fa-download"></i> <span class="loader" style="display: none;"><i class="fas fa-spinner fa-spin"></i></span>
                                    </button>


                                    <a href="#" 
                                    class="btn btn-secondary btn-sm file-viewer-btn" 
                                    data-file-name="<?= htmlspecialchars($revision['filename'], ENT_QUOTES) ?>" 
                                    data-file-id="<?= htmlspecialchars($revision['file_id'], ENT_QUOTES) ?>">
                                        <i class="fas fa-eye"></i> 
                                    </a>


                                    <a href="http://localhost/word_reader/pdf_viewer.php?file_id=${(revision.file_id)}" class="btn btn-secondary btn-sm" target="_blank"> 
                                        <i class="fas fa-eye"></i> 
                                    </a>
                ...
                if (response.status === 'success') {
                    ...
                    response.data.forEach(revision => {
                        ...
                        if (fileExtension === 'pdf') {
                            ...
                            // Word file viewer
                            viewerButton = `
                                <a href="http://localhost/word_reader/read_word.php?file_id=${revision.file_id}" class="btn btn-secondary btn-sm" target="_blank"> 
                                    <i class="fas fa-eye"></i> 
                                </a>`;
...
<script>
  $(document).ready(function () {
    ...
    $('.edit-file').on('click', function () {
        ...
        // Fetch revisions via AJAX
        $.get('../functions/rp_proposal_berf_files/fetch_file_revisions.php', { file_id: fileId }, function (response) {
            if (response.status === 'success') {
                ...
                response.data.forEach(revision => {
                    revisionsHtml += `
                        <tr>
                            ...
                            <td>${revision.file_size}</td>
                            <td>
                                <button class="btn btn-warning btn-sm edit-revision" data-id="${revision.id}">
    ...
    $('#editRevisionForm').on('submit', function (e) {
        ...
        $.ajax({
            url: '../functions/rp_proposal_berf_files/update_revision_file.php', // Backend script to handle file upload and update
            method: 'POST',
            data: formData,
...
<script>
  document.getElementById("addFileForm").addEventListener("submit", function (e) {
    ...
    fetch("../functions/rp_proposal_berf_files/add_file.php", {
        method: "POST",
        body: formData,
    })
...
$('#addRevisionForm').on('submit', function (e) {
    ...
    $.ajax({
        url: '../functions/rp_proposal_berf_files/add_revision_file.php', // Backend script to handle file upload and version increment
        method: 'POST',
        data: formData,
        processData: false, // Required for file uploads
        contentType: false, // Required for file uploads
...
<script>
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: '../functions/rp_proposal_berf_files/delete_file.php',
                method: 'POST',
                data: { file_id: fileId },
                success: function (response) {
                    if (response.status === 'success') {
