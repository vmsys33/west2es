...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Check for duplicates in t_lr_files table (existing logic)
        $stmt = $pdo->prepare("
        ...
        if ($duplicateCountInAdminFiles > 0) {
            echo json_encode(['status' => 'error', 'message' => 'Duplicate file found in the database (t_lr_files or t_lr_files_versions).']);
            $pdo->rollBack();
            exit;
        }

        // Additional check for duplicates in t_lr_files_versions table
        $stmt = $pdo->prepare("SELECT COUNT(*) AS duplicate_count FROM t_lr_files_versions WHERE filename = :filename");
        ...
        if ($duplicateCountInAdminFilesVersions > 0) {
            echo json_encode(['status' => 'error', 'message' => 'Duplicate file found in t_lr_files_versions table.']);
            $pdo->rollBack();
            exit;
        }
        ...
        $file_path2 = '/west2es/uploads/files/t_lr_files/' . basename($file['name']);
