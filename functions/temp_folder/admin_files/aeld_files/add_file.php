...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Check for duplicates in aeld_files table (existing logic)
        $stmt = $pdo->prepare("
            SELECT COUNT(*) AS duplicate_count 
            FROM (
                SELECT filename FROM aeld_files
                UNION ALL
                SELECT filename FROM aeld_files_versions
            ) AS combined
        ...
        if ($duplicateCountInAdminFiles > 0) {
            echo json_encode(['status' => 'error', 'message' => 'Duplicate file found in the database (aeld_files or aeld_files_versions).']);
            $pdo->rollBack();
            exit;
        }

        // Additional check for duplicates in aeld_files_versions table
        $stmt = $pdo->prepare("SELECT COUNT(*) AS duplicate_count FROM aeld_files_versions WHERE filename = :filename");
        ...
        if ($duplicateCountInAdminFilesVersions > 0) {
            echo json_encode(['status' => 'error', 'message' => 'Duplicate file found in aeld_files_versions table.']);
            $pdo->rollBack();
            exit;
        }
