...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Check for duplicates in if_proposals_files table (existing logic)
        $stmt = $pdo->prepare("
            SELECT COUNT(*) AS duplicate_count 
            FROM (
                SELECT filename FROM if_proposals_files
                UNION ALL
                SELECT filename FROM if_proposals_files_versions
            ) AS combined
        ...
        if ($duplicateCountInAdminFiles > 0) {
            echo json_encode(['status' => 'error', 'message' => 'Duplicate file found in the database (if_proposals_files or if_proposals_files_versions).']);
            $pdo->rollBack();
            exit;
        }

        // Additional check for duplicates in if_proposals_files_versions table
        $stmt = $pdo->prepare("SELECT COUNT(*) AS duplicate_count FROM if_proposals_files_versions WHERE filename = :filename");
        ...
        if ($duplicateCountInAdminFilesVersions > 0) {
            echo json_encode(['status' => 'error', 'message' => 'Duplicate file found in if_proposals_files_versions table.']);
            $pdo->rollBack();
            exit;
        }
        ...
        $file_path2 = '/west2es/uploads/files/if_proposals_files/' . basename($file['name']);

        date_default_timezone_set('Asia/Manila');
        $currentDatetime = date('Y-m-d H:i:s');

        $table1 = 'if_proposals_files';
