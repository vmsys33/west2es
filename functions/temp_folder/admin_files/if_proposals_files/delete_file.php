...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['file_id']) && is_numeric($_POST['file_id'])) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("SELECT filename FROM if_proposals_files_versions WHERE file_id = :file_id");
            ...
            foreach ($files as $file) {
                $filePath = '../../uploads/files/if_proposals_files/' . $file['filename']; // Adjust the path as needed
            ...
            // Delete from if_proposals_files_versions
            $stmt1 = $pdo->prepare("DELETE FROM if_proposals_files_versions WHERE file_id = :file_id");
            $stmt1->execute(['file_id' => $file_id]);

            // Delete from if_proposals_files
            $stmt2 = $pdo->prepare("DELETE FROM if_proposals_files WHERE id = :file_id");
