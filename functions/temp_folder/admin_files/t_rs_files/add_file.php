...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Check for duplicates in t_rs_files table (existing logic)
        $stmt = $pdo->prepare("
            SELECT COUNT(*) AS duplicate_count 
            FROM (
                SELECT filename FROM t_rs_files
                UNION ALL
                SELECT filename FROM t_rs_files_versions
            ) AS combined
        ...
        if ($duplicateCountInAdminFiles > 0) {
            echo json_encode(['status' => 'error', 'message' => 'Duplicate file found in the database (t_rs_files or t_rs_files_versions).']);
            $pdo->rollBack();
            exit;
        }

        // Additional check for duplicates in t_rs_files_versions table
        $stmt = $pdo->prepare("SELECT COUNT(*) AS duplicate_count FROM t_rs_files_versions WHERE filename = :filename");
        ...
        if ($duplicateCountInAdminFilesVersions > 0) {
            echo json_encode(['status' => 'error', 'message' => 'Duplicate file found in t_rs_files_versions table.']);
            $pdo->rollBack();
            exit;
        }
        ...
        $file_path2 = '/west2es/uploads/files/t_rs_files/' . basename($file['name']);
