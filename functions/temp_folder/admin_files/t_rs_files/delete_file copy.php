...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['file_id']) && is_numeric($_POST['file_id'])) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("SELECT filename FROM t_rs_files_versions WHERE file_id = :file_id");
            $stmt->execute(['file_id' => $file_id]);
            $files = $stmt->fetchAll(PDO::FETCH_ASSOC);

            // Loop through all revision files and delete them
            foreach ($files as $file) {
                $filePath = '../../uploads/files/t_rs_files/' . $file['filename']; // Adjust the path as needed
            ...
            // Delete from t_rs_files_versions
            $stmt1 = $pdo->prepare("DELETE FROM t_rs_files_versions WHERE file_id = :file_id");
            $stmt1->execute(['file_id' => $file_id]);

            // Delete from t_rs_files
            $stmt2 = $pdo->prepare("DELETE FROM t_rs_files WHERE id = :file_id");
