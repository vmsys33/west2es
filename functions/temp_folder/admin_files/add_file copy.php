...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Check for duplicates in admin_files and admin_files_versions based on filename
        $stmt = $pdo->prepare("
            SELECT COUNT(*) AS duplicate_count 
            FROM (
                SELECT filename FROM admin_files
                UNION ALL
                SELECT filename FROM admin_files_versions
            ) AS combined
