...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['file_id']) && is_numeric($_POST['file_id'])) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("SELECT filename FROM t_pp_files_versions WHERE file_id = :file_id");
            ...
            foreach ($files as $file) {
                $filePath = '../../uploads/files/t_pp_files/' . $file['filename']; // Adjust the path as needed
            ...
            // Delete from t_pp_files_versions
            $stmt1 = $pdo->prepare("DELETE FROM t_pp_files_versions WHERE file_id = :file_id");
            $stmt1->execute(['file_id' => $file_id]);

            // Delete from t_pp_files
            $stmt2 = $pdo->prepare("DELETE FROM t_pp_files WHERE id = :file_id");
