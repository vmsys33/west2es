...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        // Update file details
        $stmt = $pdo->prepare("UPDATE t_pp_files SET filename = :filename WHERE id = :file_id");
        $stmt->bindParam(':filename', $fileName);
        $stmt->bindParam(':file_id', $fileId, PDO::PARAM_INT);
        $stmt->execute();

        // Update latest datetime in t_pp_files_versions
        $stmt = $pdo->prepare("UPDATE t_pp_files_versions SET datetime = :datetime WHERE file_id = :file_id ORDER BY version_no DESC LIMIT 1");
