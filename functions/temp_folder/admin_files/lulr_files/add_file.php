...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Check for duplicates in lulr_files table (existing logic)
        $stmt = $pdo->prepare("
            SELECT COUNT(*) AS duplicate_count 
            FROM (
                SELECT filename FROM lulr_files
                UNION ALL
                SELECT filename FROM lulr_files_versions
            ) AS combined
        ...
        if ($duplicateCountInAdminFiles > 0) {
            echo json_encode(['status' => 'error', 'message' => 'Duplicate file found in the database (lulr_files or lulr_files_versions).']);
            $pdo->rollBack();
            exit;
        }

        // Additional check for duplicates in lulr_files_versions table
        $stmt = $pdo->prepare("SELECT COUNT(*) AS duplicate_count FROM lulr_files_versions WHERE filename = :filename");
        ...
        if ($duplicateCountInAdminFilesVersions > 0) {
            echo json_encode(['status' => 'error', 'message' => 'Duplicate file found in lulr_files_versions table.']);
            $pdo->rollBack();
            exit;
        }
        ...
        $file_path2 = '/west2es/uploads/files/lulr_files/' . basename($file['name']);
