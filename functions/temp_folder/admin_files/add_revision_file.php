...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $fileFilename = $_POST['file_name']; // Get the file ID from the form
    ...
    try {
        // Check for duplicate filenames in admin_files_versions table
        $stmt = $pdo->prepare("SELECT COUNT(*) AS duplicate_count FROM admin_files_versions WHERE filename = :filename");
        ...
        if ($duplicateCount > 0) {
            echo json_encode(['status' => 'error', 'message' => 'Duplicate file found in admin_files_versions table.']);
            exit;
        }

        // Fetch the current latest version number for the file
        $stmt = $pdo->prepare("SELECT name, MAX(version_no) AS latest_version FROM admin_files_versions WHERE file_id = :file_id");
