...
$currentPageNoExt = pathinfo($currentPage, PATHINFO_FILENAME);

$file = $currentPageNoExt;

// Call the function to get the page title
$pageTitle = getPageTitle($currentPage);

$table1 = $file;
$table2 = $file . '_versions';
...
$stmt = $pdo->prepare("
    ...
    WHERE 
        af.status = 'approve' -- Filter for rows where {$table1}.status is 'approve'
    GROUP BY 
    ...
    <tbody>
        ...
        <?php foreach ($files as $file): ?>
            <tr>
                ...
                <td>
                    <button class="btn btn-info btn-sm preview-file" data-id="<?= $file['id'] ?>" data-table="<?= $table2?>" data-bs-toggle="modal" data-bs-target="#previewFileModal">
                        <i class="fas fa-eye"></i>
                    </button>
                    <button class="btn btn-success btn-sm edit-file" data-id="<?= $file['id'] ?>" data-table="<?= $table2?>" data-version="<?= $file['version_no'] ?>" data-bs-toggle="modal" data-bs-target="#editFileModal">
                    ...
                    <button class="btn btn-danger btn-sm delete-file" data-id="<?= $file['id'] ?>" data-table1="<?= $table1 ?>" data-table2="<?= $table2 ?>" data-version="<?= $file['version_no'] ?>" >
