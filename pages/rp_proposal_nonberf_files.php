...
$stmt = $pdo->prepare("
    SELECT 
        ...
        afv.datetime
    FROM 
        rp_proposal_nonberf_files af
    LEFT JOIN 
        rp_proposal_nonberf_files_versions afv 
        ON af.id = afv.file_id
    LEFT JOIN 
        user_data ud 
        ON af.user_id = ud.id_no
    WHERE 
        af.status = 'approve' -- Filter for rows where rp_proposal_nonberf_files.status is 'approve'
    GROUP BY 
    ...
    <tbody>
        ...
        <?php foreach ($files as $file): ?>
            <tr>
                ...
                <td>
                    <button class="btn btn-info btn-sm preview-file" data-id="<?= $file['id'] ?>" data-table="<?= 'rp_proposal_nonberf_files_versions'?>" data-bs-toggle="modal" data-bs-target="#previewFileModal">
                        <i class="fas fa-eye"></i>
                    </button>
                    <button class="btn btn-success btn-sm edit-file" data-id="<?= $file['id'] ?>" data-table="<?= 'rp_proposal_nonberf_files_versions'?>" data-version="<?= $file['version_no'] ?>" data-bs-toggle="modal" data-bs-target="#editFileModal">
                    ...
                    <button class="btn btn-danger btn-sm delete-file" data-id="<?= $file['id'] ?>" data-table1="<?= 'rp_proposal_nonberf_files' ?>" data-table2="<?= 'rp_proposal_nonberf_files_versions' ?>" data-version="<?= $file['version_no'] ?>" >
            ...
            <div class="modal-body">
                <form id="addFileForm" enctype="multipart/form-data" action="../functions/file_functions/add_file.php" method="POST">
                    <div class="mb-4">
                        ...
                        <input type="hidden" name="table1" value="rp_proposal_nonberf_files">
                        <input type="hidden" name="table2" value="rp_proposal_nonberf_files_versions">
                ...
                response.data.forEach(revision => {
                    revisionsHtml += `
                        <tr>
                            ...
                            <td>${revision.file_size}</td>
                            <td>
                                <button class="btn btn-warning btn-sm edit-revision" data-id="${revision.file_id}" data-table1="rp_proposal_nonberf_files" data-table2="rp_proposal_nonberf_files_versions"
                                ...
                                `<button class="btn btn-danger btn-sm delete-revision" 
                                    data-table1="rp_proposal_nonberf_files" data-table2="rp_proposal_nonberf_files_versions" 
