    ...
    <tbody>
        ...
        <?php foreach ($files as $file): ?>
            <tr>
                ...
                <td>
                    <button class="btn btn-info btn-sm preview-file" data-id="<?= $file['id'] ?>" data-table="<?= 'admin_files_versions'?>" data-bs-toggle="modal" data-bs-target="#previewFileModal">
                        <i class="fas fa-eye"></i>
                    </button>
                    <button class="btn btn-success btn-sm edit-file" data-id="<?= $file['id'] ?>" data-table="<?= 'admin_files_versions'?>" data-version="<?= $file['version_no'] ?>"data-bs-toggle="modal" data-bs-target="#editFileModal">
                    ...
                    <?php if ($_SESSION['user_role'] !== 'faculty'): ?>
                        <button class="btn btn-danger btn-sm delete-file" data-id="<?= $file['id'] ?>" data-table1="<?= 'admin_files' ?>" data-table2="<?= 'admin_files_versions' ?>">
            ...
            <div class="modal-body">
                <form id="addFileForm" enctype="multipart/form-data" action="../functions/file_functions/add_file.php" method="POST">
                    <div class="mb-4">
                        ...
                        <input type="hidden" name="table1" value="admin_files">
                        <input type="hidden" name="table2" value="admin_files_versions">
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                ...
                method: 'POST',
                data: { file_id: fileId, file_table1: fileTable1, file_table2: fileTable2 },
                success: function (response) {
                    if (response.status === 'success') {
