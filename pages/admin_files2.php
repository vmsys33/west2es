...
$stmt = $pdo->prepare("
    ...
    FROM 
        admin_files af
    LEFT JOIN 
        admin_files_versions afv 
        ON af.id = afv.file_id
    LEFT JOIN 
        user_data ud 
        ON af.user_id = ud.id_no
    WHERE 
        af.status = 'approve' -- Filter for rows where admin_files.status is 'approve'
    GROUP BY 
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="addFileForm" enctype="multipart/form-data" action="../functions/admin_files/add_file.php" method="POST">
                    <div class="mb-4">
...
<script>
    $(document).ready(function () {
        ...
        $('.preview-file').on('click', function () {
            ...
            $.get('../functions/admin_files/fetch_revisions.php', { file_id: fileId }, function (response) {
...
<script>
  $(document).ready(function () {
    ...
    $('.edit-file').on('click', function () {
        ...
        // Fetch revisions via AJAX
        $.get('../functions/admin_files/fetch_file_revisions.php', { file_id: fileId }, function (response) {
            if (response.status === 'success') {
    ...
    $('#editRevisionForm').on('submit', function (e) {
        ...
        $.ajax({
            url: '../functions/admin_files/update_revision_file.php', // Backend script to handle file upload and update
            method: 'POST',
            data: formData,
...
$('#addRevisionForm').on('submit', function (e) {
    ...
    $.ajax({
        url: '../functions/admin_files/add_revision_file.php', // Backend script to handle file upload and version increment
        method: 'POST',
        data: formData,
        processData: false, // Required for file uploads
        contentType: false, // Required for file uploads
...
<script>
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: '../functions/admin_files/delete_file.php',
                method: 'POST',
                data: { file_id: fileId },
                success: function (response) {
                    if (response.status === 'success') {
